...
class ReLeaSE(AbstractGenerator):
    ...
    def __init__(self,generator_properties):
        ...
        initial_state = "release/checkpoints/generator/checkpoint_biggest_rnn"
    ...
    def generate_smis(self, n_to_generate=None):
        """
        ...
        if n_to_generate is None:
            n_to_generate = self.batch_size
        generated = self.generator.generate(n_to_generate, verbose=1)
        return generated

    def generate_mols(self, n_to_generate=None):
        """Generate molecules using the generator, and returns them as RDKit molecules."""
        generated = self.generate_smis(n_to_generate)
        mols = [Chem.MolFromSmiles(smi) for smi in generated]
        return mols
    
    def bias_generator(self, estimator, ctrl_opts={}):
        """Bias the generator.
        ...
        # Clean up possible old files fro previous runs
        if Path('./chk').is_dir():
            for file in Path("./chk").glob("*"):
                file.unlink()
            Path('./chk').rmdir()
        Path('./chk').mkdir()
        ...
        # General options
        verbosity = 0
        if 'verbosity' in ctrl_opts.keys():
        ...
        # History File
        # -------------------------------------------
        if 'history_file' in ctrl_opts.keys():
            history_file = Path(ctrl_opts['history_file'])
        else:
            history_file = "./chk/biasing_history.csv"
            Path("./chk").mkdir(exist_ok=True, parents=True)
        ...
        restart = False
        if 'restart' in ctrl_opts.keys():
            restart = ctrl_opts['restart']
        if restart:
        ...
        # bw completely unbiased. Otherwise, they are from the
            ...
            for reinforcement_iteration in range(gen_start, gen_start + max_iterations):
                ...
                for policy_iteration in range(n_policy):
                    ...
                    if verbosity > 1:
                        ...
                        utils.print_stats(predictions_cur, LENGTH_LIM=60, header="PROPERTIES", print_header=True)
                        utils.print_stats(rewards_cur, LENGTH_LIM=60, header="REWARDS", print_header=True)
                    ...
                    if n_approved_mols > 0:
                        ...
                        if (new_mols_since_last_train) > n_best:
                            ...
                            if (seed_smi is not None 
                                and isinstance(seed_smi,str)
                                and f"<{seed_smi}>" not in new_data_file):
        ...
        print(f"Final generator state saved to file {chk_path}.")

        # Clean up
        for file in Path("./chk").glob("*.smi"):
            if file.name != "generated_smiles_final.smi" and file.name != "unbiased.smi":
                file.unlink()
                
        for file in Path("./chk").glob("*.chk"):
            if file.name != "biased_generator_final.chk":
                file.unlink()

        if Path("./chk/biasing_history.csv").is_file():
            Path("./chk/biasing_history.csv").unlink()
